...
class AppTest(TestCase):
    ...
    def test_inventory_get_success(self):
        res = self.client.get('/inventory/items/')
        print(res.json())
        assert b'[]' in res.content
        assert 200 == res.status_code

    def test_inventory_post_success(self):
        data = {
            'name': 'shirt',
            'category': 'top wear',
            'price': 700,
        ...
        res = self.client.post("/inventory/items/", data=data)
        print(res.json())
        assert 'shirt' == res.json()['name']
        assert 201 == res.status_code

    def test_inventory_delete_success(self):
        self.client.post('/inventory/items/', data={
            'name': 'shirt',
            'category': 'top wear',
            'price': 700,
            'discount': 20,
    ...
    def test_inventory_post_error(self):
        res = self.client.post('/inventory/items/', data={
            'name': 'shirt',
            'category': 'top wear',
            'price': 700,
            'discount': 20,
    ...
    def test_inventory_get_sort_success(self):
        self.client.post("/inventory/items/", data={
            'name': 'shirt',
            'category': 'top wear',
            'price': 700,
    ...
    def test_inventory_get_query_category_success(self):
        self.client.post("/inventory/items/", data={
            'name': 'shirt',
            'category': 'top wear',
            'price': 1300,
